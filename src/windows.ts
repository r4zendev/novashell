...
import { type Astal, Gdk } from "ags/gtk4";
...
export class Windows extends GObject.Object {
	...
	public createWindowForMonitors(
		...
		return () => {
			const monitorIds = this.getMonitorIds();
			...
			return monitorIds.map((monId) => {
				return createRoot(() => {
					const scope = getScope();
					const instance = create(monId, scope) as Astal.Window;
	...
	// AstalHyprland's init aborts when any window has a non-UTF-8 title, leaving
	// its monitor list empty. Fall back to GDK so the shell can still spawn its
	// per-monitor windows. The `id` here is used as a GDK monitor index, which is
	// also how Hyprland monitor ids are consumed by the window widgets.
	private gdkMonitorCount(): number {
		return Gdk.Display.get_default()?.get_monitors().get_n_items() ?? 0;
	}

	public getMonitorIds(): Array<number> {
		const monitors = AstalHyprland.get_default().get_monitors();
		if (monitors.length > 0) return monitors.map((mon) => mon.id);

		return Array.from({ length: this.gdkMonitorCount() }, (_, i) => i);
	}
	...
	public getFocusedMonitorId(): number | null {
		...
		return this.gdkMonitorCount() > 0 ? 0 : null;
	}
